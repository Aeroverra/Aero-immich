...
-- the customViews preference group (lock trigger; no migration, preferences are JSON)
UPDATE "user_metadata"
   SET "value" = "value" - 'customViews'
 WHERE "key" = 'preferences'
   AND "value" ? 'customViews';

-- -----------------------------------------------------------------------------
-- 5j. Album view state (1789670000000-CustomViewsAlbumState): which albums the
--     default view hides for clients without the views flag, and the trigger
--     that keeps album.updatedAt when only the sync cursor is bumped.
-- -----------------------------------------------------------------------------
DROP TABLE IF EXISTS "album_view_state";
DROP TABLE IF EXISTS "album_view_state_checkpoint";
DROP TRIGGER IF EXISTS "album_updatedAt_preserve" ON "album";
DROP FUNCTION IF EXISTS preserve_updated_at();
...
DELETE FROM "migration_overrides"
 WHERE "name" IN (
   ...
   'function_preserve_updated_at',
   'trigger_album_updatedAt_preserve'
 );
...
DELETE FROM "kysely_migrations"
 WHERE "name" IN (
   ...
   '1789670000000-CustomViewsAlbumState'
 );
  ...
  FROM (
    ...
    SELECT 'migration_overrides ' || name
     ...
     WHERE name LIKE '%\_private\_%' ESCAPE '\'
        OR name IN ('function_tag_delete_audit', 'function_tag_asset_delete_audit', 'function_tag_asset_after_insert', 'function_tag_asset_after_delete', 'function_view_delete_audit', 'function_view_tag_delete_audit', 'trigger_view_delete_audit', 'trigger_view_updatedAt', 'index_view_ownerId_isDefault_uidx', 'trigger_tag_delete_audit', 'trigger_tag_asset_after_delete', 'trigger_tag_asset_after_insert', 'trigger_tag_asset_delete_audit', 'trigger_view_tag_delete_audit', 'function_preserve_updated_at', 'trigger_album_updatedAt_preserve')
    UNION ALL
    SELECT 'kysely_migrations ' || name
     ...
     WHERE name LIKE '%PrivateMode%'
        ...
        OR name LIKE '%-CustomViewsAlbumState'
    UNION ALL
    ...
    SELECT 'user_metadata preferences.' || k || ' for user ' || "userId"
      FROM user_metadata, unnest(ARRAY['stacks', 'autoStack', 'stackActions', 'customViews']) AS k
     WHERE key = 'preferences'
       AND value ? k
    UNION ALL
    ...
    SELECT 'table ' || table_name
      FROM information_schema.tables
     WHERE table_schema = current_schema()
       AND table_name IN ('view', 'view_tag', 'view_audit', 'view_tag_audit', 'tag_audit', 'tag_asset_audit', 'album_view_state', 'album_view_state_checkpoint')
    UNION ALL
    SELECT 'column ' || table_name || '.' || column_name
      FROM information_schema.columns
     WHERE table_schema = current_schema()
    ...
    SELECT 'function ' || proname
     ...
     WHERE pronamespace = current_schema()::regnamespace
       AND proname IN ('tag_delete_audit', 'tag_asset_delete_audit', 'tag_asset_after_insert', 'tag_asset_after_delete',
                       'view_delete_audit', 'view_tag_delete_audit', 'preserve_updated_at')
    UNION ALL
    SELECT 'type ' || typname
      FROM pg_type
     WHERE typnamespace = current_schema()::regnamespace
