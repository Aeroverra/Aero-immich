...
deleted_rows=$(query "SELECT count(*) FROM kysely_migrations WHERE name LIKE '%AssetDeletedChecksum%'")
[[ "$deleted_rows" -ge 1 ]] || fail "the fork image applied no AssetDeletedChecksum migration"
[[ "$(query "SELECT count(*) FROM information_schema.tables WHERE table_name='asset_deleted_checksum'")" == 1 ]] || fail "asset_deleted_checksum table missing after the fork boot"
log "fork: $deleted_rows AssetDeletedChecksum migration row(s), asset_deleted_checksum table present"
...
# seed a user whose preferences carry the fork's privateMode and deletedReimport
# blocks next to an upstream key, so the revert has something to strip and
# something to keep, plus one remembered checksum in the fork's table
...
query "INSERT INTO user_metadata (\"userId\", key, value) VALUES ('$uid', 'preferences', '{\"privateMode\":{\"timeoutMinutes\":5},\"deletedReimport\":{\"mode\":\"skip\"},\"memories\":{\"enabled\":false}}'::jsonb)
...
[[ "$(query "SELECT count(*) FROM user_metadata WHERE key='preferences' AND value ? 'deletedReimport'")" == 1 ]] || fail "seeding the deletedReimport preference failed"
query "INSERT INTO asset_deleted_checksum (\"ownerId\", checksum, \"assetId\", \"originalFileName\") VALUES ('$uid', decode('deadbeef', 'hex'), gen_random_uuid(), 'revert.jpg')" >/dev/null
[[ "$(query "SELECT count(*) FROM asset_deleted_checksum")" == 1 ]] || fail "seeding a remembered checksum failed"
log "fork: seeded user $uid with privateMode and deletedReimport preferences and one remembered checksum"
stop_server
group_end
...
[[ "$(query "SELECT count(*) FROM kysely_migrations WHERE name LIKE '%AssetDeletedChecksum%'")" == 0 ]] || fail "AssetDeletedChecksum row still in kysely_migrations"
[[ "$(query "SELECT count(*) FROM information_schema.tables WHERE table_name='asset_deleted_checksum'")" == 0 ]] || fail "asset_deleted_checksum table still present"
[[ "$(query "SELECT count(*) FROM user_metadata WHERE key='preferences' AND value ? 'deletedReimport'")" == 0 ]] || fail "deletedReimport still in user preferences"
...
log "user preferences: privateMode and deletedReimport stripped, other keys kept; asset_deleted_checksum dropped"
group_end
...
tables=$(psql_db -c '\dt')
grep -q 'asset_deleted_checksum' <<<"$tables" && fail "\\dt still lists asset_deleted_checksum"
log "\\dt has no asset_deleted_checksum table"
stop_server
group_end
